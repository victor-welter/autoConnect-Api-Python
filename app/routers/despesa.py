...
async def create_despesa(despesa_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.put("update_despesa/{id_despesa}", status_code=200)
async def update_despesa_route(id_despesa: int, despesa_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.delete("/delete_despesa/{id_despesa}", status_code=200)
