...
from app.schemas.tipo_despesa import TipoDespesaSchema
...
async def create_tipo_despesa(tipo_despesa_data: TipoDespesaSchema, db: Session = Depends(get_db)):
    try:
...
@router.put("/update_tipo_despesa", status_code=200)
async def update_tipo_despesa_route(id_tipo_despesa: int, tipo_despesa_data: TipoDespesaSchema, db: Session = Depends(get_db)):  
    try:
