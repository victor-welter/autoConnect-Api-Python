...
async def create_categoria(categoria_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.put("update_categoria/{id_categoria}", status_code=200)
async def update_categoria_route(id_categoria: int, categoria_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.delete("/delete_categoria/{id_categoria}", status_code=200)
