...
async def create_permissao(permissao_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.put("update_permissao/{id_permissao}", status_code=200)
async def update_permissao_route(id_permissao: int, permissao_data: dict, db: Session = Depends(get_db)):
    try:
...
@router.delete("/delete_permissao/{id_permissao}", status_code=200)
